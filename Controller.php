...
class Addressbook_Controller extends Tinebase_Controller_Abstract implements Tinebase_Event_Interface, Tinebase_Container_Interface
{
    ...
    public function handleEvents(Tinebase_Event_Abstract $_eventObject)
    {
    	...
    	if($_eventObject instanceof Billing_Events_SetAccountsBankTransferDetected){
    		try{
    			Addressbook_Controller_Contact::onSetAccountBankTransferDetected($_eventObject);
    		}catch(Exception $e){
				echo $e->__toString();
				//print_r($_eventObject);
				throw $e;
    		}
    	}
    }
    ...
    /**
     * 
     * Get the contact image as a tempfile
     * @param 	int 	$_identifier	Contact ID
     * @param 	string 	$tempImagePath	Filepath for tempfile
     * @return	string	Tempfile name
     */
    public function getImageAsTempFile( $_identifier, $tempImagePath){
     	$image = $this->getImage($_identifier);
     	$mime = $image->__get('mime');
     	$ext = $image->getImageExtension();
     	$tempFileName = $tempImagePath.md5($_identifier).$ext;
     	
     	file_put_contents($tempFileName, $image->getBlob($mime));
     	return $tempFileName;
    }
